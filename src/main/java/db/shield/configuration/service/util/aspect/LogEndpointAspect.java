package db.shield.configuration.service.util.aspect;
...
public class LogEndpointAspect {
    ...
    @Before("execution(* db.shield.configuration.service.controller..*(..))")
    public void logBefore(JoinPoint joinPoint) {
        log.info("Entering method: {} with arguments: {}", joinPoint.getSignature().toShortString(), joinPoint.getArgs());
    }

    @AfterReturning(pointcut = "execution(* db.shield.configuration.service.controller..*(..))", returning = "result")
    public void logAfterReturning(JoinPoint joinPoint, Object result) {
        log.info("Exiting method: {} with result: {}", joinPoint.getSignature().toShortString(), result);
    }

    @AfterThrowing(pointcut = "execution(* db.shield.configuration.service.controller..*(..))", throwing = "error")
